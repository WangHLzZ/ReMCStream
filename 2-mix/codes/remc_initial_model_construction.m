function [Model,labeledData]=remc_initial_model_construction(data,train_class_labels,numClass,clsSize)
...
%     labeledData{idxClass} = [];
    labeledData{idxClass}=train_data(train_data_labels==(idxClass-1),:);
    totalRows = size(labeledData{idxClass}, 1);
    if  totalRows > clsSize
        startIndex = totalRows - clsSize + 1;
        labeledData{idxClass} = labeledData{idxClass}(startIndex:end, :);
        assert(size(labeledData{idxClass}, 1) == clsSize);
    end
end
