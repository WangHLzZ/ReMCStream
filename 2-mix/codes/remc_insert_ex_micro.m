    function [ Model_temp ] = remc_insert_ex_micro(Model_temp,ex,idx,currentTime)
    ...
    if ex.label_flg==1 && Model_temp{idx,10}~=1
        Model_temp{idx,4}=ex.label;
        Model_temp{idx,5}=1;%label flg
        Model_temp{idx,10}=1;
    elseif ex.reliable_plable~=-1 && Model_temp{idx,5}==0
        Model_temp{idx,4}=ex.reliable_plable;
        Model_temp{idx,5}=1;%label flg
        Model_temp{idx,10}=0;
    end
